#!/usr/bin/env python3
"""
Complete video processing pipeline for BIRS videos (CPU MODE - RELIABLE)
...
def transcribe_audio(wav_path: str) -> str:
    """Transcribe audio using Whisper large-v3 on CPU (slower but reliable)"""
    logger.info(f"Loading Whisper large-v3 (CPU mode)...")
    model = WhisperModel("large-v3", device="cpu", compute_type="int8")

    logger.info(f"Transcribing: {wav_path}")
    logger.info("Using CPU - this will take ~10-15 minutes for 60-min video")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python process_video_cpu.py <video_url> <basename>")
        print("Example: python process_video_cpu.py https://videos.birs.ca/2025/25w5318/202507210900-Walton.mp4 25w5318_202507210900_Walton")
        sys.exit(1)
